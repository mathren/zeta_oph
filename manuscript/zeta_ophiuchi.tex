...
\todo{smooth below}
After presenting the evolution of the masses and mass transfer rate in
\Secref{sec:MT}, we discuss in \Secref{sec:rot} the internal and
surface rotation of our accretor model, and in \Secref{sec:mixing} its
composition. As a brief summary, the accretor in our binary accretes
about 3.4\Msun\ material during mass transfer (out of
$\sim 10.6$\Msun\ total transferred). This causes rejuvenation and
comparing to the single stellar tracks in the HR diagram
(\Figref{fig:HRD_both}) the star appears to be X Myr after detachment,
instead of the actual 7.3 Myr. After detachment, the core and the
envelope are disconnected and the core rigidly rotates rapidly at a
roughly constant spin rate until TAMS, while the envelope spins down
but also rotates rigidly. } \YG{Here, I am thinking we can check what
is the age of the 20\Msun single star model that the track overlaps
with at detachment. }

 As a brief summary of the results presented there, we
find that both the surface rotation and surface composition of \zoph\
are more easily explained by our accretor model than by single
rotating stellar models.

The accretor starts as a
...
initial rotation rates), hence their TAMS profile are
...
presence of convective boundary mixing. In our model, this is
initially provided by overshooting and to a lesser extent dynamical
shear (between the top and second panel of \Figref{fig:D_mix})
