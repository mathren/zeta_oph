...
(dark blue), rotational mixing (light blue), and thermohaline mixing
...
of the overshooting region), where dynamical shear can dominate due to
the post-RLOF contraction and spin-up of the core (see \Secref{sec:rot}).
...
causes a meridional circulation in the envelope. A small sub-surface
...
In the third panel, the star has already accreted roughly the total amount that it will, including CNO-enriched layers. Thermohaline mixing takes over the dominant role in the envelope (although meridional circulations persist behind it). The increase in core-mass causing the rejuvenation effect is still going on, and the mixing at the core-envelope boundary is partially still due to dynamical shear, but an off center convective region also appears. We do not include
over/undershooting for off-center convective layers, which could increase the coupling between this layer and the core.

By the fourth panel, roughly corresponding to \zoph's structure today,
thermohaline mixing is progressively shutting down, but some residual
mixing at the core edge still remains.

Finally, the last panel shows that has the post-RLOF evolution
proceeds, a long-lived convective region $\sim 1\,M_\odot$ wide
develops above the core. This can affect the density profile at the
base of the envelope. If the binary system remains bound after the
explosion of the companion and goes through a common envelope
\citep[e.g.,][]{paczynski:76}, this layer will be crucial to determine
the success or failure of the common envelope ejection.

We emphasize that the core growth due to the accretion of mass, the
consequent ingestion in the burning region of fresh H-rich material, and the
consequent rejuvenation are strongly dependent on the presence of
...
\citetalias{villamariz:05} suggesting that maybe our model has evolved
slightly too much compared to \zoph. On the other hand, the age of
$8.5$\,Myr is on the lower hand of the range comparible with \zoph\
...
$0.005\pm0.004$.  By the accretor's TAMS, rotational mixing (in the
