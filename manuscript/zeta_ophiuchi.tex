...
circulations and thermohaline mixing, (ii) outward mixing of the
...
\Figref{fig:struct_rot}). Rigid rotation persists for
...
The impact of mass transfer on the accretor stars in massive binaries
has received little attention so far. We have ran self-consistent
spherically-symmetric \texttt{MESA} calculations of massive binary
systems evolving both stars simultaneously and coupled, including
parameter variations to test the robustness of our results to varying
algorithmic physics.  As a first
application, we focused on finding a model qualitatively matching
observations of the nearest O-type star to Earth, the runaway \zoph,
assuming its runaway nature is explained by the binary SN scenario
\citep{blaauw:61}.
...
$Z=0.01$ (see \Secref{sec:best_model}). We computed this fiducial
model with standard stellar physics
...
This binary system experiences stable thermal-timescale Roche lobe
...
from the surface inwards by meridional circulations and
...
as they mix upward $^{14}\mathrm{N}$ from their core. The treatment of
