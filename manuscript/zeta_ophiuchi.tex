...
\begin{abstract}
  ...
  impact of mass accretion on the surface composition, internal
  ...
  $\zeta$ Ophiuchi, which has long been proposed to have accreted mass
  before being ejected as a runaway by the companion's supernova.  We
  compare our accretor to single rotating stars and find that the
  later and stronger spin-up provided by binary interactions produces
  significant differences. Specifically, the core of the accretor
  retains higher spin at the end of the main sequence, and its inner
  density profile is significantly different because of an off-center
  convective shell persisting during and after the mass
  transfer. Moreover, in our accretor, surface pollution can occur
  because of the accreted material from the donor (which is
  progressively mixed inwards) rather than outward mixing of core
  material.  Our models show effects of mass accretion on the
  secondary star in a binary not captured in single rotating stellar
...
parameters and all our numerical results are available at
\url{https://doi.org/10.5281/zenodo.4701565}. We discuss here only the
main relevant parameters and describe the effects of varying some of
the assumed values in
...
between the hot and cold wind for intermediate $T_\mathrm{eff}$. We
assume a wind scaling factor of 1. Compared to the measurements for
...
TAMS 11.2\,Myr, similar to the lifetime of a non-rotating single
...
the accretion of material from the donor's core progressively mixed
inwards (see \Secref{sec:surf_comp}). We find that the rotation and
...
\begin{figure}[bp]
  ...
  \caption{Equatorial surface rotational velocity (top panel) and
    ...
    \cite{vink:00, vink:01}, which might be $\sim$2 orders of magnitude too
...
opacity $\tau\leq 100$. In the top panel, the dark horizontal red band corresponds to
...
impact our conclusions. The bottom panel of \Figref{fig:surf_rot}
...
for single stars, \citetalt{maeder:00}), late in its evolution (cf.\
...
GSF mixing to a lesser extent) occurs. For clarity, we do not show
