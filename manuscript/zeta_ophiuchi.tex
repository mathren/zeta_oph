...
Modeling the evolution of massive binaries is challenging because of
the intricate and coupled role of several notoriously difficult
stellar physics ingredients (e.g., rotation, mixing, high mass-loss
rates). Here we follow self-consistently the coupled evolution of two
massive stars in a binary system using \texttt{MESA} (version
15140). Our choice of input parameters and our numerical results are
available at \todo{link}. We discuss here only the main relevant
physical parameters, and Appendix~\ref{sec:software} gives more
details on our choice of input physics.

We adopt the Ledoux criterion to determine convective stability and a
mixing length parameter of $1.5$. Semiconvection \citep{langer:85} and a
thermohaline \citep{kippenhahn:83} mixing are included with an
efficiency of $1.0$ for both. We use the exponential core
overshooting from \cite{herwig:00} with free parameters
...
Rotation is initialized assuming tidal synchronization at the
beginning of the evolution. Our models include in a diffusive
approximation the effect of Eddington-Sweet circulations \citep{},
which dominates the chemical mixing due to rotation, and also SSI, DSI
and GSF \todo{sharpen and ref} \citep[e.g.,]{gotberg:17, gotberg:18}.
We assume a Spruit-Tayler dynamo for the transport of angular
momentum, and chose the same free parameters as \cite{heger:00}. This
also includes the rotational enhancement of wind mass loss as in \cite{langer:98}/
...
almost a factor of 100 \citep[weak wind problem][]{marcolino:09}.
...
donor star during RLOF, and assume that the transfered layers reach
...
We define RLOF detachment as the moment
...
sequence (TAMS) radius, and no mass is being transferred. At this
point in time, we save a model for the accreting star, and continue
its evolution until TAMS with the same setup.
...
sequence and $\sim8400$\,years after, when the donor's effective
temperature reaches about $10^4$\,K, mass transfer starts. This
results in a stable case B \citep{kippenhahn:67} Roche lobe overflow
(RLOF). We refer to \cite{gotberg:17, laplace:21, blagorodnova:21}
...
At this point, the accretor star (bottom panel) is still on the main
sequence (effective temperature $T_\mathrm{eff}\simeq10^{4.5}$\,K),
and because of the accretion of matter it quickly becomes
over-luminous ($L\simeq10^{5.4}\,L_\odot$), and its radius increases
dramatically from $\sim7.5\,R_\odot$ to $\sim35\,R_\odot$. Once the
...
surface, which causes a transient  surface convection layer. This small feature
...
\texttt{MESA} releases, because although only a very small amount
 of mass is involved, this would lead to large radial
...
\todo{logg is maybe on the low side}

\subsection{Mixing and abundances}
