...
\begin{abstract}
  ...
  O-type star to Earth, $\zeta$ Ophiuchi (\zoph), has long been proposed to be
  a product of binary evolution. Specifically, \zoph\ has likely
  accreted mass from a former companion, before being ejected at the
  time of the companion's explosion. Therefore, this star provides an
  ideal test bed for models of accretors in binaries. Despite this, most stellar models
  have tried unsuccessfully to reproduce its observable properties
  relying on single-star rotating models. We use the open-source
  stellar evolution code \texttt{MESA} to self-consistently model the
  coupled evolution of two massive stars ($M_1\gtrsim 25\,M_\odot\geq M_2$) in a binary
  evolving through a stable post-donor-main-sequence mass transfer. We find that adopting
  common assumptions for the treatment of rotation, chemical mixing,
  and mass transfer efficiency, our models of the accreting star
  reproduce reasonably well the kinematic, spectroscopic, and
  photometric properties of \zoph. We compare the internal structure
  and evolution of our accretor to fast-rotating single stars, and
  find that the late and extreme spin up by binary
  interactions produces a faster-spinning core in the accretor than in
  single stars. Our model demonstrates the impact that binary evolution
  can have on the secondary star, with possible implications for the
  spins of compact objects and the further evolution (either in a
  binary or as single stars) as supernova and/or gravitational wave progenitors.
\end{abstract}
...
post-main-sequence stable mass transfer (case B) through Roche Lobe
overflow (RLOF, \citealt{kippenhahn:67}).  Many studies
\citep[e.g.,][]{gotberg:17, gotberg:18, laplace:20, laplace:21,
  blagorodnova:21} \todo{more refs. from other groups} have focused on
the dramatic impact these interactions have on the donor star. Often
the binary companion is treated as a point mass.
...
\citep[e.g.,][]{packet:81, cantiello:07}, and possibly be polluted by
...
(and mergers) can appear as blue stragglers and thus impact cluster
...
stand out in astrometric surveys \citep[e.g.,][]{eldridge:11,
  renzo:19walk}. Assuming a constant star formation history,
\cite{renzo:19walk} estimated that $10.1^{+4.6}_{-8.6}\%$ of O-type
stars might be accretors released after a SN. %  -- where the errors span
% a range of parameter variations.

From the transients
perspective, massive accretor stars are also important: \cite{zapartas:19}
...
gravitational-wave progenitors go through a common-envelope
...
are lacking, most of the studies focus on lower mass B-type donors
...
challenging to obtain. Here, we argue that the nearest O-type star to
...
\citep{walker:79, vink:09}, making it an Oe star. Its surface rotation
rate is extremely large, with most estimates of the projected
rotational velocity from optical spectra exceeding
$v\sin(i)\gtrsim 400\,\kms$ \citep[e.g.,][]{zehe:18}. Using optical
...
to model it rely purely on rotational mixing to explain the surface composition
...
We treat rotation in the ``shellular'' approximation of constant
rotational frequency $\omega$ along equipotential surfaces, and we assume
tidal synchronization at the beginning of our runs. For our fiducial period choice ($P=100$\,days), this effectively means both the stars in our binary are
initially slow rotators. Our models include a diffusive approximation
for Eddington-Sweet circulations
...
determined structure of the donor star. Mass transfer is
...
$T_\mathrm{eff}$ increases. At $T_\mathrm{eff}\simeq 10^{4.43}$\,K the
...
$\mu$ on top of the primordial envelope of the accretor, modifying the morphology of the evolutionary track. Thermohaline mixing starts in the outer layers of the accreting star, and, together with rotational mixing, it progressively dilutes the surface He and N mass fractions and causes noisy features on the HR diagram \citep[e.g.,][]{cantiello:07}. The (standard) algorithmic choices in modelling mixing and rotation might impact the morphology of the accretor's evolutionary track during RLOF. The entire duration of RLOF is only about
$10^4$\,years, and therefore unlikely to be observable to probe directly the accuracy of our treatment of mixing. We expand on the mixing processes inside the accretor in \Secref{sec:mixing}.
...
$v_2\simeq52\,\kms$, which decreases to
$\sim$$40\,\kms$ due to wind-driven widening of the binary at the end
of the donor's He core burning. Further decrease should be expected
because of the post-He core burning winds, but the value we obtain is
in good agreement with the presently observed runaway velocity of
\zoph.
...
second broad peak in the mass transfer rates centered at
$7.261$\,Myrs. We find this to be the culprit of the numerical
...
stars in the parent association \citep[e.g.][]{murphy:21}. Moreover,
we have assumed that elemental abundances scale with the Solar values
...
equatorial radius extend and thus the $\omega/\omega_\mathrm{crit}$
and mass transfer efficiency (see \Secref{sec:bin_param}), it
determines outward mixing from the core, and more importantly inward
mixing from the surface. We were unable to run models without
Eddington-Sweet meridional circulations to test the sensitivity of our
results to rotational mixing. \todo{check fi\_workstation/RUNS/.../ES\_10x\_lower}
...
momentum. In reality, geometric distorsion of the outer layers because
of the centrifugal forces would also occur, which is not modeled in a
1D code as \texttt{MESA}.
...
constrained. We have explored variation in these values, and the
...
disrupted at the first SN (see \Secref{sec:SN_comp} and
\citealt{renzo:19walk}). For example, taking $P$=75\,days
...
post-RLOF orbital velocity of the accretor is about $10\,\kms$ higher
than the $50\,\kms$ of our fiducial model.

Increasing the donor mass also has a similar effect on the post-RLOF
orbital velocity of the accretor. Using $M_1=30\,M_\odot$
(cf. $25\,M_\odot$ in \Secref{sec:best_model}),
$M_2=17\,M_\odot$, and $P$=100\,days, we obtain a post-RLOF velocity
of $65\,\kms$. However, this produces a stripped donor of
$\sim$16\,$M_\odot$ at RLOF detachment, with stronger wind mass
loss. Therefore this binary is expected to widen relatively more than
our fiducial model of \Secref{sec:best_model}, slowing down the
secondary. The increased mass of the stripped star could also imply a
lower chance of the star exploding (however, see
\Secref{sec:SN_comp}).
...
$\sim$$253\pm54\,\kms$, which would be sufficiently large to unbind the binary \citep{kalogera:96, tauris:15}.  The ejecta mass would depend on the post-RLOF wind mass loss of our donor star, which we do not model.  At the end of our binary model (black diamond in \Figref{fig:HRD_both}), our stripped donor is
...
is rather massive, recent studies suggest a higher ``explodability''
of donor stars in binary systems \citep[e.g.,][]{schneider:21,
  laplace:21, vartanyan:21}.

We have neglected the impact of the explosion on the structure
of the accretor star. At the time of the explosion, the accretor
subtends a solid angle $\sim$$R^2/a^2\simeq
0.05$\,degrees with $R$ the accretor radius and
$a$ the binary separation. We neglect the post-RLOF wind-driven orbital widening for this estimate.
The blast wave will hit the accretor causing mass loss -- directly via ablation and by injecting energy in the envelope, inflating it and enhancing its wind \citep{wheeler:75, tauris:98, podsiadlowski:03, hirai:18}.  Because of the SN shock, the just ejected new runaway star might appear bloated and redder (long before it overtakes the slowing SN remnant). The impact of this brief out of thermal equilibrium phase on the stellar spin should be investigated further. Using 2D hydrodynamic simulations of the star-SN ejecta interactions in close binaries ($a\lesssim
...
$\lesssim10^{-2}\,M_\odot$.  The SN ejecta might also pollute the surface of the runaway depositing processed nuclear material \citep[e.g.][]{przybilla:08}. However, enhahnced mass loss, rapid rotation and mixing might rapidly dilute the yields.


\section{Conclusions}
\label{sec:conclusions}

We have demonstrated that self-consistent 1D calculations
...
We found that it is possible to qualitatively explain the observed
features of \zoph\ with a relatively wide and massive binary
(initially $M_1=25\,M_\odot$, $M_2=17\,M_\odot$, and
$P=100$\,days). Such system experiences a dynamically stable
post-donor-main-sequence (case B) Roche lobe overflow. Standard
stellar physics assumptions for modeling rotation, mixing, and mass
transfer in binaries reproduce reasonably well the main observational
features of \zoph\ in terms of its spatial velocity, its surface
...
$^{14}\mathrm{N}$ and $^4\mathrm{He}$ rich surface composition is
\emph{not} the result of pure outward rotational mixing.  Instead,
this material is transferred from the receeding core of the donor star
and mixed inwards by rotation, and to a smaller degree, by thermohaline
mixing.  Thus, the present day surface mass fractions of \zoph\ put a
coupled constrain the mass transfer efficiency and mixing in the
accretor. Therefore, such star should \emph{not} be used to calibrate models of
...
($t\gtrsim7.25$\,Myr), allowing the star to be simultaneously nitrogen
rich and still fast rotating after the mass transfer phase. The
treatment of close-to-critical rotation impacts the accretion
efficiency, the radius evolution, and the inward mixing.
...
spin of the second compact object born in an interacting binary system.
...
The study of the evolution of the secondary stars in massive binary
systems is crucial to understand population of massive stars,
transients, and gravitational-wave progenitors: because of the binary
interaction, the evolution of the accretor can differ from a single star.
...
\acknowledgements{We are grateful to E.~Zapartas, A.~Jermyn,
  M.~Cantiello, and R.~Neu\"auser for helpful discussions.}
...
The inlists, processing scripts, and model output will be made available at~\todo{link}.
...
are $\sim 20\%$ higher in the high resolution/low \texttt{mesh\_delta\_coeff} runs. The left panel
