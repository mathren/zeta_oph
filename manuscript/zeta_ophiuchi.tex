...
($\lesssim 10^4\,M_\odot$) but can expand to significantly large
...
\begin{figure}[htbp]
  ...
  \caption{Mixing diffusion coefficients in the accretor star. The
    ...
    (point G). A movie of the entire evolution is available at \url{https://github.com/mathren90/zeta_oph/blob/main/movies/movie_nice/mixing_movie.mp4}.}
  \label{fig:D_mix}
\end{figure}
...
(dark blue), rotational mixing (yellow), and thermohaline mixing
...
and Goldreich-Schubert-Fricke instability, but throughout the evolution
...
spin-up of the core (see \Secref{sec:rot}) can lead to significant dynamical shear.
...
the convective core initially extending to mass coordinate
...
In the third panel, the star has already accreted all the mass that it will, including CNO-enriched layers from the donor's interior. Thermohaline mixing takes over the dominant role in the envelope (although meridional circulations persist behind it). The increase in core-mass causing the rejuvenation effect is still going on, and the mixing at the core-envelope boundary is partially still due to dynamical shear, but an off center convective region also appears. We do not include
over/undershooting for off-center convective layers, which could increase the coupling between this layer and the core. By the fourth panel, roughly corresponding to \zoph's structure today,
thermohaline mixing is progressively shutting down, but some residual
mixing at the core edge still remains.
...
$\sim$$1\,M_\odot$ wide above the core, but disconnected from it. This is a consequence of core growth because of accretion, and can affect the density profile at the base of the envelope.
...
kinematic age. The surface rotational velocity in excess
...
the adopted $Z=0.01$ is shown by the ZAMS profiles in purple.
...
mixing of CNO-processed material
from the donor star by meridional circulations and thermohaline mixing, (ii) outward
mixing of the CNO-processed material from the accretor's core caused
by rotational mixing triggered by the accretion-driven spin-up.
...
% projected rotational velocity exceed $v\sin(i)\simeq 400\,\kms$,
...
\todo{maybe move to discussion these two paragraphs below}

\Figref{fig:rho} shows a density comparison between the accretor in the last panel of \Figref{fig:D_mix} and for initially
$20\,M_\odot$ single rotating stars. These were computed with the same setup except for the initial rotation rate, initialized as labelled. If the binary system remains bound after the explosion of the companion, and the evolutionary expansion of the accretor leads to a common envelope \citep[e.g.,][]{paczynski:76}, the layer above the He core is crucial to determine the success or failure of the common envelope ejection.

Common envelope simulations have so far neglected the impact of
previous RLOF phase(s) on the density structure of the stars
initiating the dynamically unstable mass transfer. Depending on the post-main-sequence
evolution of this model, this effect might be important in determining
...
\begin{figure}[htbp]
  ...
  \caption{Comparison of the density profiles of the accretor model
    ...
    $X_c=0.085$. The inset magnifies the region above the core, where the outcome of common envelope events is
