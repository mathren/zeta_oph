...
So far, the impact of mass transfer on the accretor star in massive
binaries has received little attention.  To investigate this, we have ran \texttt{MESA}
calculations of massive binaries evolving two coupled stars
simultaneously. We also explored parameter variations
(\Secref{sec:discussion}) to test the robustness of our results with
respect to algorithmic physics.
...
star ejected from a binary at the core-collapse of the former
...
rotation (see \Secref{sec:methods} and
Appendix~\ref{sec:software}).

Our fiducial model (see \Secref{sec:best_model}) is a binary starting
with $M_1=25\,M_\odot$, $M_2=17\,M_\odot$, and $P=100$\,days at
metallicity $Z=0.01$ (see \Secref{sec:best_model}). We also consider
variations in the initial parameters and in the implementation of
physical processes, discussed in \Secref{sec:discussion}. Less massive
accretors remain too cool throughout the evolution to be compatible
with \zoph, and more equal initial mass ratios lead to a more evolved
accretor at the onset of mass-transfer, again resulting in too cool
temperatures. Increasing the donor's initial mass might result in
stripped stars unlikely to form a neutron star in their final SN
explosion.

Our fiducial binary experiences stable thermal-timescale Roche lobe
overflow after the end of the main sequence of the donor (case
B). Our model of the accretor compares rather well with observations
of \zoph\ about $1.5-2$\,Myrs after the end of mass transfer,
corresponding to the remaining donor's lifetime at the end of our
simulations plus the kinematic age of \zoph. Specifically,
the position on the HR diagram, the runaway space velocity (estimated
based on the accretor's orbital velocity), the
...
Our model of \zoph\ differs significantly from previous studies:
...
from the surface inwards into the accretor by meridional circulations and
...
in the accretor. \zoph\ should \emph{not} be used to
calibrate models of rotational mixing in single star models.
...
\subsection{Accretors are not like single rotating stars}

Our models also highlight some general differences between accretors in massive
binaries and stars evolving as single throughout their life.

\begin{itemize}
\item core spin
\item above core structure and implications for CE
\end{itemize}
...
% profile. The spin up of the accreting star occurs late in its
% evolution ($t\gtrsim7.25$\,Myr for our fiducial binary), from the
% surface, and reaches to critical surface rotation
% $\omega/\omega_\mathrm{crit}\simeq 1$. This allows the accretor star
% to be simultaneously $^{14}\mathrm{N}$ rich and still fast rotating
% after the mass transfer phase, while single rotating stars spin down
% as they mix upward $^{14}\mathrm{N}$ from their core. %REPETITION % The treatment of
% % close-to-critical rotation impacts the accretion efficiency, the
% % radius evolution, and the inward mixing.

% Single-stars rotating rapidly from birth are initialized as
...
% injected late and from the surface. When this angular momentum is
...
% accretor star in an interacting binary
% system.

% Another important difference is the density structure (cf.\ \Figref{fig:rho}):\todo{finish}
