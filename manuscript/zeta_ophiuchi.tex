...
\begin{abstract}
  Most massive stars are born in binaries close enough for
  mass-transfer to occur. This modifies the appearance, internal
  structure, and future evolution of both stars, however the effects
  on the accretor are often overlooked. We model the evolution of a
  100-day period binary consisting initially of a 25\Msun\ and a
  17\Msun, which experiences stable mass transfer. We focus on the
  impact of mass accretion on the surface abundances, rotation rate,
  and internal structure of the accretor. To anchor our models, we
  show that our accretor star broadly reproduces the properties of
  $\zeta$ Ophiuchi, a runaway thought to be the product of the
  disruption of a former binary system.  We compare our accretor to
  single rotating stars and find that the later and stronger spin-up
  provided by binary interactions produces significant
  differences. Specifically, the core of the accretor retains a much
  higher spin, and its inner density profile is significantly
  different because of the appearance of an off-center convective
  shell during the mass transfer, which remains for the rest of the
  main sequence. Moreover, in our accretor surface pollution can
  occur because of the accreted material from the donor (which is
  progressively mixed inwards) rather than outward mixing of core
  material.  Our models demonstrate the impact of mass accretion on
  the secondary star in a binary that is not captured in single
  rotating stellar models, with possible implications for its further
  evolution (either in a binary or as single stars), the final
  collapse, and the resulting spins of the compact object formed.
\end{abstract}
...
from D-E, the donor star briefly expands again.  This is due to the
...
point the donor is less massive than the accretor and the binary orbit
...
\citealt{neuhauser:20}). The post-RLOF orbital velocity of the
...
\begin{figure}[bp]
  ...
  \caption{Equatorial surface rotational velocity (top panel) and
    ...
    lines). At $\sim{}7.25$\,Myr the mass transfer quickly spins up
    the accretor to critical rotation: the dashed horizontal line in
    the bottom panel shows the upper-limit we impose. By the time the
    donor detaches from the RLOF the accretor is still spinning at
    $\sim{}400\,\kms$. From this point (diamonds) onwards, we continue
    the evolution as a single star, and the accretor spins down
    because of wind mass loss.  Note however that we use the wind
...
agreement with the observationally constrained values for typical Be
stars \citep[see][for a review]{rivinius:13}.

Close to the end of the main sequence, the increase in wind mass loss
rate strengthens the surface spin-down. This effect is also seen in
the late main-sequence evolution of single stars rotating rapidly at
birth.
...
panels highlights matter accreted during RLOF.  We use colors to show
the age of the star at each profile shown. The accretor starts as a
17$\,M_\odot$ and is rejuvenated by binary interactions (see
\Secref{sec:mixing}), reaching TAMS at 11.2\,Myr. For comparison, the
...
initial rotation rates), hence their TAMS profile are a shade of
purple rather than pink in \Figref{fig:struct_rot}.
...
the overshooting extension to $\sim{}9\,M_\odot$. The slow rotation
imparted by the assumed initial tidal synchronization causes a
meridional circulation in the envelope. Meridional circulations are
also present in the core throughout the evolution, but with a
diffusivity more than nine orders of magnitude lower than core
convection. A small sub-surface convective zone is also appreciable at
the very surface (see, e.g., \citealt{cantiello:21}).
...
will, including CNO-enriched layers from the donor's
interior. Thermohaline mixing takes over the dominant role in the
...
region has also appeared. Because we adopt an exponentially decreasing
...
core and the off-center convective region is weak. We do not include
...
\YG{A convective layer above the hydrogen burning core that develops
  ...
  models at Z=0.01 (see however REFs). Apart from affecting the
  composition profile and therefore also the density profile of the
  star if it develops in a region that previously belonged to the
  main-sequence core, it is possible that it affects the future
  evolution of the star. Convective layers in this region of a star
  have been associated with late expansion and helium core burning as
  blue supergiants (Schootemeijer+19, Klencki+20a), which in turn can
  hamper future binary interaction or cause nuclear timescale mass
  transfer and long lasting X-ray binary phases
  (Marchant+21). Considering the role of accretor stars similar to our
  model as progenitors of merging binary neutron stars (Tauris+17,
  Vigna-Gomez+20), it will be important to account for these
  structural changes when investigating the evolutionary pathways
  resulting in gravitational wave events. Post-main sequence evolution
  of the accretor star is a next step and the topic of our future
  study. % Do I promise too much? Maybe yes...
} \YG{Note: we probably want to cite Groh+19, just haven't read it
...
The core growth caused by the accretion of mass, the consequent
ingestion in the burning region of fresh H-rich material, and the
resulting rejuvenation of the accretor are strongly dependent on the
presence of convective boundary mixing. \YG{I don't understand what is
  the convective boundary mixing - do you mean the convective layer
  above the core?} In our model, this is initially provided by
...
layer above the overshooting region dominates this mixing. \YG{Are you
  absolutely sure about the origin of this convective layer? I have
  had many discussions about convective layers similar to this one and
  I have not yet been convinced by any explanation for its
  origin. This could be different. What proves that it was provoked by
  overshooting and shear? Wait, I think I may be misunderstanding what
  we are talking about? }

Instead, in the envelope, Eddington-Sweet circulations first dominate,
then thermohaline mixing takes over -- roughly from the kink after
point C in \Figref{fig:HRD_both}, corresponding to the accretion of
CNO-processed material, until the end of RLOF. The combination of
these two processes mixes CNO-processed material from the donor star
from the surface inward, and CNO-processed material from the core of
the accretor outward. \YG{Do we want to show a composition profile?
  This sounds like the composition of the accretor star is not as the
  initial composition in any part of the star - is that correct? It
  may show interesting gradients too... }

% Summary about the rejuvenation and mass accretion extent
\YG{In summary, the star accretes about 3.4\Msun\ material during mass
  transfer (out of $\sim 10.6$\Msun\ total transferred). This causes
  rejuvenation and comparing to the single stellar tracks in the HR
  diagram (\Figref{fig:HRD_both}) the star appears to be X Myr after
  detachment, instead of the actual 7.3 Myr. After detachment, the
  core and the envelope are disconnected and the core rigidly rotates
  rapidly at a roughly constant spin rate until TAMS, while the
  envelope spins down but also rotates rigidly. } \YG{Here, I am
  thinking we can check what is the age of the 20\Msun single star
  model that the track overlaps with at detachment. }
...
parameter variations for each individual star in the binary. We
discuss parameters governing the mass transfer phase in
...
at higher $Z$ (see also \Secref{sec:bin_init}).
...
also mix efficiently with the core, enhancing further the rejuvenation
effect
...
values, and the qualitative behavior of the models is similar.
...
disrupted at the first SN (see \Secref{sec:SN_comp}). For example,
taking $P$=75\,days (cf.\ 100\,days in our fiducial model), the binary
still experiences stable case B mass transfer, but the post-RLOF
orbital velocity of the accretor is about $60\,\kms$, that is
$\sim{}10\,\kms$ higher than in our fiducial model, because of the
larger orbital velocity in the tighter binary system.
...
relatively high $T_\mathrm{eff}$ are difficult to reproduce assuming
initially less massive accretors (which would remain too cool even
after accreting mass), or more equal initial mass ratio (which would
...
donor star, which is very uncertain \citep[e.g.,][]{renzo:17, vink:17,
  gilkis:19}. \YG{Vink 2017 and Gilkis+19 don't say that the wind mass
  loss rate for stripped stars are uncertain, but they create/use a
  new prescription for wind mass loss rate. I would suggest a simple
  change to "for a comparison of different wind mass loss rates, see
...
surface of the runaway depositing processed nuclear material
...
rotating and $^{14}\mathrm{N}$-rich, while single rotating stars can
only mix upward $^{14}\mathrm{N}$ from their core, and have
significantly spun down by the time they become
$^{14}\mathrm{N}$-rich. \YG{I am not convinced by this argument -
  looking at \Figref{fig:n14}, the fast rotating single stars seem to
  have enriched surfaces already after 4Myr or so (second line in the
  diagram, orange color), then their surfaces become slightly more
  enriched with time, but not much. From the veq-t diagram, it doesn't
  seem to me that they are spun down at that time.  } Moreover,
alternative scenarios where \zoph\ evolved as a single, fast-rotating
star require ad-hoc explanations for the runaway velocity, and have
been shown by \citetalias{villamariz:05} to struggle in reproducing
surface mass fractions, apparent age, mass, and rotation rate
simultaneously.
