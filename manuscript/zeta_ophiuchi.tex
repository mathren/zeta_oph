...
fraction will directly interact with the companion(s) before their
death \citep[e.g.,][]{sana:12}. The most common type of interaction is a post-main-sequence
...
rotation at its surface \citep[e.g.,][]{packet:81, cantiello:07}, and
...
O-type that accreted mass earlier on. The majority of these will be
...
after a SN -- where the errors span a range of binary evolution
...
\citep[e.g.,][]{belczynski:16nat, tauris:17, law-smith:20,
  broekgaarden:21}. It is therefore possible that accretion of mass
...
Despite their importance, accretor stars in binaries have
so far received much less attention than the donor stars, with the
pioneering work of \cite{hellings:83, hellings:84} and \cite{braun:95}
as notable exceptions. Large grids of accretor models are missing, and
only sparse models exist \citep[e.g.,][]{cantiello:07}\todo{more refs.}. This is because of the complexity of these
models, where one needs to follow in detail the \emph{coupled}
evolution of two rotating stars exchanging mass, and the admittedly
large number of free parameters involved to model each individual star
and their interactions. Here, we will argue that the nearest O-type
star to Earth, $\zeta$ Ophiuchi\footnote{also known as HD\,149\,757.}
provides a unique opportunity to constrain these models, and present
the first fully coupled binary evolution calculations with donor
masses in excess of $20\,M_\odot$ taylored to explain the properties
of this star.

\zoph\ has a spectral type O9.5{\rm IVnn} \citep{sota:14} and
occasionally also shows emission lines \citep{vink:09}, and a distance
from Earth of $\sim$110\,pc \citep[e.g.,][and references
...
\cite{claret:17}. We also use the experimental superadiabaticity
reduction \texttt{MLT--} introduced in \texttt{MESA} 15140.
