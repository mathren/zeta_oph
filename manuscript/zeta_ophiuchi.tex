...
\citep[e.g.,][]{gotberg:17, gotberg:18, laplace:20,
  laplace:21, blagorodnova:21}
\todo{more refs. from other groups} have
focused on the dramatic impact of these interactions on the initially
more massive donor star. Often the closest binary companion is treated
as a point mass.
...
From the transients
perspective, massive accretors stars are also important: \cite{zapartas:19}
...
compact object \citep[e.g.,][]{belczynski:16nat, tauris:17, broekgaarden:21}. Therefore, it is possible that accretion of mass
...
\citep[e.g.,][]{cantiello:07}. This is because of the complexity of
modeling these, which requires following in detail the \emph{coupled}
...
particular mass transfer). We report here on exploration of parameter
variations for each individual star, and discuss parameters governing the mass
transfer phase in \Secref{sec:bin_param}.
...
Rotation is a critical ingredient of our models. \todo{no Eddington
...
the mass transfer \citep[e.g.,][]{vanrensbergen:06}. In our models it
depends on the radial and rotational evolution of the accreting
star. During RLOF,
the accretor is out of gravothermal equilibrium with significant
impact on its radius and ultimately on the amount of mass transferred
and its angular momentum. In reality, geometric distorsion of the
outer layers because of the centrifugal forces would also occur, which
is not modeled in a 1D code as \texttt{MESA}.
...
Decreasing the initial period results in larger post-RLOF orbital
...
material to the orbit or the donor's spin. Since
...
$\sim$$253\pm54\,\kms$, which would be sufficiently large to unbind the binary.  The ejecta mass would depend on the post-RLOF wind mass loss of our donor star, which we do not model.  At the end of our binary model (black diamond in \Figref{fig:HRD_both}), our stripped donor is
